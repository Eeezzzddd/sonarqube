...
export default class AdvancedTimeline extends React.PureComponent {
  ...
  getScales = () => {
    ...
    const flatData: Array<Point> = flatMap(this.props.series, (serie: Serie) => serie.data);
    return {
  ...
  renderEvents = (xScale: Scale, yScale: Scale) => {
    ...
    //$FlowFixMe eventSize is always defined
      ...
      <g>
        {events.map((event, idx) => (
          <rect
            ...
            width={eventSize}
            height={eventSize}
            x={xScale(event.date) - offset}
            y={yScale.range()[0] - offset}
          />
        ))}
      </g>
    );
